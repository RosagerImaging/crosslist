...
chrome.runtime.onMessage.addListener((
  ...
  if (message && typeof message === 'object' && message.type === 'EXTENSION_STATUS_REQUEST') {
    sendResponse({
  ...
  if (message.type === 'CAPTURE_MARKETPLACE_SESSION') {
    handleCredentialCapture(message, sendResponse);
    return true;
  }
...
async function handleCredentialCapture(
  message: { payload: { marketplace: 'ebay' | 'poshmark' } },
  sendResponse: (response: any) => void
) {
  try {
    ...
    const webAppTabs = tabs.filter(t => t.url && (t.url.startsWith('http://localhost:3000') || t.url.startsWith('https://crosslist.app')));
    ...
    console.log(`Sent encrypted credentials for ${marketplace} to ${webAppTabs.length} tabs.`);
...
async function handleAuthStateSync(
  ...
  try {
    ...
    if (session) {
      ...
      await chrome.storage.local.remove(['supabase_session_encrypted', 'auth_last_synced']);
