...
import { Button } from '@/components/ui/button' // Assuming UI components exist or using standard HTML if not
// Note: Adapting to standard HTML if UI lib components not explicitly known, but standard nextjs apps usually have them.
// I'll use standard elements with Tailwind classes to be safe, or generic structure.
// Wait, "apps/web/package.json" showed tailwind.
...
export function MarketplaceConnectionCard({ marketplace, initialStatus }: MarketplaceConnectionCardProps) {
  ...
  useEffect(() => {
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (message?.type === 'CREDENTIAL_CAPTURED' && message.payload?.marketplace === marketplace) {
  ...
  const handleConnect = async (credentials: string) => {
    setIsLoading(true)
    setError(null)
  ...
  const startConnectFlow = () => {
    ...
    setError(null)
    const url = marketplace === 'ebay' ? 'https://www.ebay.com' : 'https://poshmark.com'
    // Open in new tab
